...
function asistencias($periodo){
    switch ($periodo){
      ...
      case 4:lineap04('presente',5);lineap04('ausente',6);lineap04('retardo',7);break;
    }
}
...
echo'<form id="form1" name="form1" method="post" action="grabaralumnopk1.php">
  ...
  <table id="tblUser" width="100%" align="center" cellpadding="0" cellspacing="0">
    <thead>
        ...
        for($ii=1;$ii<=15;$ii++){linea($ii,$periodoactivo,0);}
        titulo(2);
        for($ii=16;$ii<=29;$ii++){linea($ii,$periodoactivo,0);}
        titulo(3);
        for($ii=30;$ii<=40;$ii++){
   if ($ii==36){linea($ii,$periodoactivo,1);}else{
    linea($ii,$periodoactivo,0);}
          }
        titulo(4);
        for($ii=41;$ii<=46;$ii++){linea($ii,$periodoactivo,0);}
        echo '
        <tr>
            <td></td>
            <td colspan="4"></td>
